<template>
  <div class="campaign-list-page">
    <div class="title-tab">
      Request Approval - Delete Campaign
    </div>
          ...
          <tr v-for="(data, idx) in listData" :key="idx">
            ...
            <td>
              ...
              <button
                ...
                :disabled="isBtnDisabled(data.is_delete_approved)"
              >
                Accept Campaign
              </button>
              <button
  ...
  methods: {
    ...
    clickCampaign(type, campaignId) {
      if(type === 'view') {
        ...
        Swal.fire({
          title: "Konfirmasi",
          text: "Apakah anda ingin menyetujui penghapusan campaign ini?",
          icon: "question",
          ...
          .then(
            (conf) => {
              if(conf.isConfirmed) {
                Axios.put(`campaign/approve_delete_campaign/${campaignId}`)
                  .then(
                    (res) => {
                      if(res.data.success) {
                        Swal.fire({
        ...
        Swal.fire({
          title: "Konfirmasi",
          text: "Apakah anda ingin menolak penghapusan campaign ini?",
          icon: "question",
          showCancelButton: true
        })
          .then(
            (conf) => {
              if(conf.isConfirmed) {
                Axios.put(`campaign/reject_delete_campaign/${campaignId}`)
                  .then(
                    (res) => {
                      if(res.data.success) {
                        Swal.fire({
